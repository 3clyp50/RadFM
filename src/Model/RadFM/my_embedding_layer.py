...
class MyEmbedding(nn.Module):
    def __init__(self, num_embeddings=32000, embedding_dim=5120, perceiver_num=32,vis_dim = 768, patch_size=32, frame_patch_size = 4 ,seg_channel = 256):
        ...
        # Lazy loading for MedKEBERT - only load when needed
        self.bert_tokenizer = None
        self.bert_model = None
        self._bert_loaded = False
        self.bert_projection_fc = nn.Linear(768, vis_dim)
        
        # Initialize vision encoder with memory optimization
    ...
    def _load_bert_if_needed(self):
        """Lazy load BERT models only when needed to save memory"""
        if not self._bert_loaded and self.bert_tokenizer is None:
            print("Loading MedKEBERT models on demand...")
            try:
                ...
                print("MedKEBERT models loaded successfully")
            except Exception as e:
                print(f"Error loading MedKEBERT: {e}")
                # Create dummy models if loading fails
                self._bert_loaded = False
                print("Using dummy BERT models")
    ...
    def forward(self, text_input, vision_x, key_words_query = None):
        if self.flag == 'Text':
            ...
            # Use gradient checkpointing for vision encoder to save memory
            ...
            if key_words_query != None:
                # Only load BERT models if needed for key_words_query
                self._load_bert_if_needed()
                
                if self._bert_loaded and self.bert_tokenizer is not None:
            ...
            # Use gradient checkpointing for perceiver to save memory
            with torch.cuda.amp.autocast() if torch.cuda.is_available() else torch.no_grad():
